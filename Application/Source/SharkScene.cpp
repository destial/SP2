...
SharkScene::SharkScene() {}

SharkScene::~SharkScene() {}

void SharkScene::Init()
{
...
void SharkScene::RenderMesh(Mesh* mesh, bool enableLight)
{
...
void SharkScene::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SharkScene::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
...
void SharkScene::RenderMeshOnScreen(Mesh* mesh, float size, float x, float y) {
	if (!mesh || mesh->textureID <= 0) //Proper error check
		return;
...
void SharkScene::Update(double dt)
{
	Mouse mouse;
	Update(dt, mouse);
}

void SharkScene::RenderSkybox() {
	modelStack.PushMatrix();
